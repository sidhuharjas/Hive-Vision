# Limelight 3A — set up ball detection, step by step
...
4. Set **Runtime engine** to **CPU**. Leaving it on "Coral" is the number-one
...
The configured device name must byte-for-byte match the name in
...
public void loop() {
    ...
    // report data that passed the staleness gate.
